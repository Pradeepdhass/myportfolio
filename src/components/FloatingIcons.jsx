...
const FloatingIcons = () => {
  ...
  useEffect(() => {
    ...
    const global = {
      baseAmplitude: 90, // Increased for wider movement
      baseSpeed: 0.006,  // Increased for faster floating
    ...
    const animate = () => {
      ...
      items.forEach(item => {
        ...
        const t = time * omega;
        // Added complex sine wave interaction for organic, unpredictable movement
        const offX = (Math.sin(t + phase) * ax * dir) + (Math.sin(t * 0.7) * (ax * 0.5));
        const offY = (Math.cos(t + phase * 0.8) * ay * dir) + (Math.sin(t * 0.4) * (ay * 0.5));
